...
// With minor modification, this is based on:
// https://github.com/google-deepmind/mujoco_mpc/blob/main/mjpc/simulate.cc
...
void InitializeProfiler(mj_app::SimulateBase* sim) {
...
void UpdateProfiler(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d) {
  ...
  int nisland = 0;//  mjMIN(d->solver_nisland, mjNISLAND);
...
void ShowProfiler(mj_app::SimulateBase* sim, mjrRect rect) {
...
// init sensor figure
void InitializeSensor(mj_app::SimulateBase* sim) {
...
void UpdateSensor(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d) {
  mjvFigure& figsensor = sim->figsensor;
...
// show sensor figure
void ShowSensor(mj_app::SimulateBase* sim, mjrRect rect) {
...
static void LoadScrubState(mj_app::SimulateBase* sim) {
...
static void mjui0_update_section(mj_app::SimulateBase* sim, int section) {
  mjui_update(section, -1, &sim->ui0, &sim->uistate, &sim->platform_ui->mjr_context());
}

// prepare info text
void UpdateInfoText(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d,
                    char (&title)[mj_app::SimulateBase::kMaxFilenameLength],
                    char (&content)[mj_app::SimulateBase::kMaxFilenameLength]) {
  char tmp[20];
  ...
  mju::sprintf_arr(
      ...
      solerr, solver_niter, fps, 100 * d->maxuse_arena / (double)(d->narena), mju_writeNumBytes(d->narena));
...
// update watch
void UpdateWatch(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d) {
  // clear
...
void MakePhysicsSection(mj_app::SimulateBase* sim, int oldstate) {
  ...
  for (int i = 0; i < sim->ui0.sect[SECT_PHYSICS].nitem; i++) {
    ...
    if (it->type == mjITEM_SEPARATOR) {
      if (mju::strcmp_arr(it->name, "Actuator Group Enable") &&
          mju::strcmp_arr(it->name, "Contact Override") && mju::strcmp_arr(it->name, "Physical Parameters")) {
...
void MakeRenderingSection(mj_app::SimulateBase* sim, const mjModel* m, int oldstate) {
...
void MakeVisualizationSection(mj_app::SimulateBase* sim, const mjModel* m, int oldstate) {
...
void MakeGroupSection(mj_app::SimulateBase* sim, int oldstate) {
...
void MakeJointSection(mj_app::SimulateBase* sim, int oldstate) {
...
void MakeControlSection(mj_app::SimulateBase* sim, int oldstate) {
  mjuiDef defControl[] = {
...
void MakeUiSections(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d) {
...
void AlignAndScaleView(mj_app::SimulateBase* sim, const mjModel* m) {
  // use default free camera parameters
  mjv_defaultFreeCamera(m, &sim->cam);
}

// copy qpos to clipboard as key
void CopyPose(mj_app::SimulateBase* sim, const mjModel* m, const mjData* d) {
...
mjtNum Timer() {
  static auto start = mj_app::SimulateBase::Clock::now();
  auto elapsed = Milliseconds(mj_app::SimulateBase::Clock::now() - start);
...
void CopyCamera(mj_app::SimulateBase* sim) {
...
void UpdateSettings(mj_app::SimulateBase* sim, const mjModel* m) {
  // physics flags
  for (int i = 0; i < mjNDISABLE; i++) {
...
int UiPredicate(int category, void* userdata) {
  mj_app::SimulateBase* sim = static_cast<mj_app::SimulateBase*>(userdata);
...
void UiLayout(mjuiState* state) {
  mj_app::SimulateBase* sim = static_cast<mj_app::SimulateBase*>(state->userdata);
...
void UiEvent(mjuiState* state) {
  ...
  if (state->type == mjEVENT_SCROLL && state->mouserect == 3) {
    ...
    if (sim->m_ && !sim->is_passive_) {
      ...
      mjv_moveCameraFromState(&sim->scnstate_, mjMOUSE_ZOOM, 0, -zoom_increment * state->sy, &sim->scn,
                              &sim->cam);
    }
    return;
  }
  ...
  if (state->type == mjEVENT_MOVE && state->dragrect == 3 && (sim->m_ || sim->is_passive_)) {
    ...
    if (sim->pert.active) {
      if (!sim->is_passive_) {
        mjv_movePerturb(sim->m_, sim->d_, action, state->dx / r.height, -state->dy / r.height, &sim->scn,
                        &sim->pert);
      } else {
        mjv_movePerturbFromState(&sim->scnstate_, action, state->dx / r.height, -state->dy / r.height,
                                 &sim->scn, &sim->pert);
      }
    } else {
      if (!sim->is_passive_) {
        ...
        mjv_moveCameraFromState(&sim->scnstate_, action, state->dx / r.height, -state->dy / r.height,
                                &sim->scn, &sim->cam);
      }
    }
    return;
...
SimulateBase::SimulateBase(std::unique_ptr<mujoco::PlatformUIAdapter> platform_ui, mjvCamera* cam,
...
#undef X
  ...
  if (pending_.select) {
    ...
    int selbody = mjv_select(m_, d_, &this->opt, static_cast<mjtNum>(r.width) / r.height,
                             ...
                             (pending_.select_state.y - r.bottom) / r.height, &this->scn, selpnt, &selgeom,
                             &selflex, &selskin);
...
void SimulateBase::LoadMessageClear(void) {
  {
    MutexLock lock(mtx);
    this->loadrequest = 0;
  }
...
void SimulateBase::RenderLoop() {
  frames_ = 0;
  last_fps_update_ = mj_app::SimulateBase::Clock::now();
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    ...
    auto now = mj_app::SimulateBase::Clock::now();
...
void SimulateBase::AddToHistory() {
  if (history_.empty()) {
    return;
  }
...
void SimulateBase::UpdateHField(int hfieldid) {
  MutexLock lock(this->mtx);
...
void SimulateBase::UpdateMesh(int meshid) {
  MutexLock lock(this->mtx);
...
void SimulateBase::UpdateTexture(int texid) {
  MutexLock lock(this->mtx);
